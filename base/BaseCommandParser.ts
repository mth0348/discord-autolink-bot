...
export class BaseCommandParser implements ICommandParser {
  ...
  public isAllowedCommand(message: Message | PartialMessage): boolean {
    let isCommand = this.prefixes.some((prefix) => this.discordService.checkIsCommand(message, `${Constants.prefix}${prefix}`));
    if (isCommand) {
      if (this.allowedChannels === undefined || this.allowedRoles === undefined) return true;
