    ...
    public parseSyntax(parsable: MtgParsable, card: MtgCard, addDot: boolean = false): void {
        ...
        while (text.indexOf("(") >= 0) {
            ...
            if (text.indexOf("(keyword)") >= 0) {
                const keyword = this.mtgDataRepository.getKeywordsByColorAndType(card.color.split(''), "creature", 1)[0];
            ...
            if (costIndex >= 0) {
                ...
                let cost = MtgHelper.getManacost(Math.max(1, Math.round(parseFloat(score) + Random.next(-1, 1))), colors);
